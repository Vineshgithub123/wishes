...
function newShell(){
    ...
    shell.color = colors[Maths.floor(Math.random() *colors.length)];
    shells.push(shell)
}

function newPass(shell){
    var pasCount =Math.cell(MAth.pow(shell.size,2)*Math.PI);
 ...
 function Run(){
     ...
     for (let ix in shells){
      ...
      for (let ix in pass){
          ...
          pas.x -= pas.xoff;
          pas.y -= pas.yoff;
          pas.xoff -=(pas.coff * dt * 0.001);
          pas.yoff -= ((pas.yoff + 5)* dt *0.0005);
          pas.size -= (dt * 0.002 * Math.random());

          if ((pas.y > cheight)||(pas.y<-50)||
(pas.size <=0)  )    {
    pass.splice(ix,1);
